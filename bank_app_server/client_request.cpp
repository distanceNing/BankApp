...
bool ClientRequest::OnLogin()
{
    ...
    socket_->Receive(&login_info, sizeof(struct LOGIN_INFO));
    int error_type=-1;
...
bool ClientRequest::OnSearchAcc()
{
    ...
    socket_->Send(&client,sizeof(struct CLIENT_INFO));
    return true;
}
...
bool ClientRequest::OnSerAllCard()
{
    ...
    for(int i=0;i<num;i++)
    {
        ...
        socket_->Send(&all_card,sizeof(CARD_INFO));
    }
...
bool ClientRequest::OnSave()
{
    ...
    socket_->Receive(&data,sizeof(struct SAVE_INFO));
...
bool ClientRequest::OnRefresh()
{
    ...
    socket_->Send(&client_info,sizeof(struct CLIENT_INFO));
    return false;
}
...
bool ClientRequest::OnTrans()
{
    ...
    socket_->Receive(&data,sizeof(struct TRANS_INFO));
...
bool ClientRequest::OnExit()
{
    ...
    char fromIP[20];
    UINT fromPort;
    socket_->GetPeerName(fromIP, fromPort);
    std::cout << "IP :" << fromIP << "---  Port : " << fromPort << "---Exit!" << std::endl;
    if(socket_->CloseSocket())
        return true;
    return false;
}
